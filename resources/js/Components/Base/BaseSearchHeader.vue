<template>
  <div class="relative w-full max-w-lg search-container">
    ...
    <div class="flex items-center justify-start px-2 py-1 mt-2 rounded-full bg-gray-100">
      <!-- Ícono de búsqueda -->
      <span class="material-symbols-outlined text-gray-500 text-md ml-4">search</span>
      ...
      <div :class="searchQuery.length > 0 ? 'block' : 'hidden'" class="flex justify-center items-center">
        <span @click="clearSearch" class="material-symbols-outlined cursor-pointer">close</span>
      </div>
    </div>
...
const isDropdownVisible = ref(false); // Para controlar la visibilidad de las opciones
...
const filterOptions = () => {
  if (searchQuery.value.length > 0) {
    ...
    isDropdownVisible.value = true; // Mostrar las opciones cuando se filtran
  } else {
    filteredOptions.value = [];
    isDropdownVisible.value = false; // Ocultar las opciones si no hay texto
  }
};
...
const clearSearch = () => {
  searchQuery.value = "";
  filteredOptions.value = [];
  isDropdownVisible.value = false; // Ocultar las opciones
};
...
const handleClickOutside = (event) => {
  ...
  if (!searchContainer.contains(event.target)) {
    isDropdownVisible.value = false; // Ocultar las opciones si se hace clic fuera
  }
};
